  ...
  <div class="about" id="about" name="about">
    ...
    <div v-if="wh" class="about-content">
      ...
      <el-card class="su_card">
        <h4>已发布商品</h4>
        <div class="sun_sp">
          <el-card :body-style="{ padding: '0px' }" class="aaww" v-for="item in arr" :index="item.id" :key="item.id">
            <img :src="$target+item.imgUrl"
                 class="image">
            <div style="padding: 14px;">
              <span>{{ item.goodsName }}</span>
              <div class="bottom clearfix">
                <time class="time" style="color: #ffc504">{{ item.goodsDiscount }}元</time>
                <div>
                  <router-link :to="{ path: '/goods/details', query: {productID:item.id} }" tag="a" style="color: blue">
                    查看&nbsp;&nbsp;&nbsp;&nbsp;
                  </router-link>
                  <router-link :to="{ path: '/addIdle', query: {goodsId:item.id,isType: 0}}" tag="a"
                               style="color: blue">编辑
                  </router-link>
                </div>
              </div>
